...
import com.wenjelly.maker.meta.enums.FileGenerateTypeEnum;
...
public class MetaValidator {
    ...
    public static void validAndFillModelConfig(Meta meta) {
        Meta.ModelConfigBean modelConfig = meta.getModelConfig();
        if (modelConfig == null) {
            return;
        }
        // modelConfig 默认值
        List<Meta.ModelConfigBean.ModelInfo> modelInfoList = modelConfig.getModels();
        if (!CollectionUtil.isNotEmpty(modelInfoList)) {
            return;
        }
        for (Meta.ModelConfigBean.ModelInfo modelInfo : modelInfoList) {
            // 为 group，不校验
    ...
    public static void validAndFillFileConfig(Meta meta) {
        // fileConfig 默认值
        Meta.FileConfigBean fileConfig = meta.getFileConfig();
        if (fileConfig == null) {
            return;
        }
        // sourceRootPath：必填
        ...
        // fileInfo 默认值
        List<Meta.FileConfigBean.FileInfo> fileInfoList = fileConfig.getFiles();
        if (!CollectionUtil.isNotEmpty(fileInfoList)) {
            return;
        }
        for (Meta.FileConfigBean.FileInfo fileInfo : fileInfoList) {
    ...
    public static void validAndFillMetaRoot(Meta meta) {
        ...
        String author = StrUtil.emptyToDefault(meta.getAuthor(), "wenjelly");
        String basePackage = StrUtil.blankToDefault(meta.getBasePackage(), "com.wenjelly");
